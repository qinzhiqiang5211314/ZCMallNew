...
@property (atomic, MB_STRONG) UIView *blurView;
...
//        UIView *view = [[[UIApplication sharedApplication].keyWindow subviews] lastObject];
        ...
        if (!_hud) {
            _hud = [[self alloc] initWithView:view];
            [view addSubview:_hud];
        }
        [view bringSubviewToFront:_hud];
    }
    return _hud;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.labelFont = DEFAULT_FONT(16.0f);
...
//        self.layer.shadowOpacity = 0.75f;
        ...
        self.blurView = [[UIView alloc] initWithFrame:CGRectZero];
        self.blurView.layer.cornerRadius = 5.0f;
//        self.blurView.layer.borderWidth = 0.6f;
//        self.blurView.layer.borderColor = [UIColor lightGrayColor].CGColor;
        self.blurView.clipsToBounds = YES;
        self.blurView.backgroundColor = RGBA(40, 40, 40, 0.8);
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    for (NSString *rectStr in self.touchableAreas){
...
- (id)initWithView:(UIView *)view {
	...
	return [self initWithFrame:CGRectMake(0, 64, APPSIZE.width, APPSIZE.height - 64)];
}
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
    } else if (mode == MBProgressHUDModeCustomViewSuccess){
        [indicator removeFromSuperview];
        customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Hud_Checkmark.png"]];
        customView.bounds = CGRectMake(0, 0, 40, 40);
        self.indicator = customView;
        [self addSubview:indicator];
    }else if (mode == MBProgressHUDModeCustomViewWarning){
        [indicator removeFromSuperview];
        customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Hud_Warning.png"]];
        customView.bounds = CGRectMake(0, 0, 40, 40);
        self.indicator = customView;
        [self addSubview:indicator];
    }
...
//    }
    ...
    if(self.blur){
        if(!self.blurView){
            self.blurView = [[WKBlurView alloc] initWithFrame:boxRect];
