...
#import "ZCEmotionTool.h"

@implementation ZCEmotionTool
+ (NSMutableAttributedString *)emotionStrWithString:(NSString *)text
{
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"MCLMLIbExpression" ofType:@"plist"];
    ...
    if (text.length>0) {
        ...
        for(NSTextCheckingResult *match in resultArray) {
            ...
            //        }
            for (int i=1; i<105; i++) {
                NSString *string=[NSString stringWithFormat:@"Expression_%d",i];
                NSString *strs=[NSString stringWithFormat:@"[%@]",dict[string]];
                if ([strs isEqualToString:subStr]) {
                    //face[i][@"png"]就是我们要加载的图片
                    //新建文字附件来存放我们的图片,iOS7才新加的对象
                    NSTextAttachment *textAttachment = [[NSTextAttachment alloc] init];
                    //给附件添加图片
